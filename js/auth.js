...
export async function registerEmail(email, pass){
  ...
  await setDoc(doc(db, 'users', user.uid), {
    uid: user.uid, email: user.email || '', displayName: user.displayName || nick, createdAt: serverTimestamp()
  }, { merge: true });
  return user;
}
...
export function mountUserBar(targetId='userBar'){
  ...
  if(!el) return;
  Object.assign(el.style,{display:'none',padding:'6px 10px',fontSize:'12px',background:'rgba(0,0,0,.35)',border:'1px solid #2b3240',borderRadius:'8px',margin:'6px 0'});
